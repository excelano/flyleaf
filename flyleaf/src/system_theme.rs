...
// `effectiveAppearance` — and on Linux it returns `None` unconditionally, so
// `egui-winit` passes that `Option` through untouched and egui falls back to
// `Theme::Dark`. Before this module every Linux user saw the dark window and
// no desktop setting could reach them: the tree drew dark with GNOME set to
// Light, and the window's own titlebar could draw light in the same frame,
// one window disagreeing with itself.
//
// The module is the one slipcase-desktop wrote first, where the behaviour was
// measured and the mapping below was settled; Tommy Flyleaf carries the same
// reasoning because it has the same toolkit and the same gap. The titlebar's
// light answer comes from `sctk-adwaita`, which spawns `dbus-send` and greps
// its output under a 100ms timeout; this asks the portal properly rather than
// shelling out beside it.
...
mod tests {
    ...
    /// anything but light gives every GNOME user who chose Light the dark
    /// window — which is the state this module replaced.
    #[test]
