...
//!
//! One renderer per TOML type rather than per schema: the tree knows no
//! vocabulary, so there is nothing to special-case and no allowlist to write.
//!
...
//! answer is the two keys its format requires and the table holding one; a
//! policy with nothing to say is `()`.
//!
//! Written in slipcase-desktop as its metadata editor and moved here on
//! 2026-09-07, which is why the measurements below name that application and
//! its containers: they were made there, and the code they are about is this.
//
...
/// The policy with nothing to say: every key is editable and every string
/// reads as it is.
impl Policy for () {}
...
/// escapes it, because one of its two protected strings is a file name and its
/// format requires a name be shown with bidirectional formatting characters
/// escaped: a payload called `report<U+202E>fdp.exe` once read
/// `reportfdp.exe` in this tree, two rows under a card that escaped it,
/// because egui gives such a character zero advance width. Found by hand on
/// Windows on 2026-08-29; the measurement lives with that policy.
///
...
fn scalar(
    ...
    let replacement = match &*v {
        ...
        Value::Integer(i) => {
            ...
            // answer; a column of one number, right-aligned while
...
mod tests {
    ...
    /// for, are tested there, on its policy.
    #[test]
...
"#;
    ...
    fn an_hour_without_its_leading_zero_is_still_a_time() {
        ...
        assert_eq!(
            read("2026-08-21T9:00:00").as_deref(),
            Some("2026-08-21T09:00:00")
        );
    ...
    /// Document order is preserved and never sorted. Authoring order
    ...
    /// it. It is shown unattached rather than dropped.
    #[test]
    ...
    /// `dotted`. Dotted keys are ordinary in a `Cargo.toml`, so this is not an edge.
    #[test]
