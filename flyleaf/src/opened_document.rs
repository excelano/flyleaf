...
//!
//! slipcase-desktop's module, with the class renamed; every measurement in
//! the comments below was made there and is recorded in its README.
//
// Author: David M. Anderson
// Built with AI assistance (Claude, Anthropic)
//
//! Linux and Windows both hand a double-clicked file to this application
...
//! blamed the application whose association was correct. This module listens.
//!
//! **This is the one place in this application that writes `unsafe`**, and
//! `main.rs` carries the `allow` that permits it. The widget and `flyleaf-core`
//! are still `#![forbid(unsafe_code)]` and nothing here is reachable from them.
//!
...
/// The file macOS last asked for, waiting to be picked up by the window.
///
/// One rather than a list. An Apple Event can carry several documents, because
/// a person can select three files and press Open, but this application
/// shows one file at a time and has nowhere to put the others. The first
...
/// How to wake the window when a file arrives.
///
/// egui draws when something happens, and an Apple Event is not something that
/// happens to egui: without this the file would sit in `ARRIVED` until a
...
/// percent-decoding, so a file with a space in its name arrives with the
...
/// there, neither a cold launch nor a file double-clicked into a running
/// window arrived. Registering **after** `eframe`'s creation closure is too
/// late for the launch itself: measured, a file double-clicked into a
...
fn register() {
    ...
    // file would be the last thing this process did. It lives as long as
...
/// `eframe` runs and the context does not exist until after. A file that
...
/// The file macOS asked for since this was last called, if any.
///
/// Taken rather than read, so that one event opens one file and the window
