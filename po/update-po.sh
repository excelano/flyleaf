...
# two sets matched exactly — measured in slipcase-desktop, where this script
# was written, and the lexing is the same here. The warnings cost nothing, but
...
#
# Both crates are scanned although only one draws anything today: `flyleaf-core`
# is the model and carries no user interface, and a message appearing there is
# either a mistake or the day that changed. Scanning it means the catalogue
# says which.
sources=$(find flyleaf/src flyleaf-core/src -name '*.rs' | sort)
...
#
# **`tr` is `t` under another name and both have to be listed.** `tree.rs` has
# called a table `t` since it was written, so it imports the lookup as `tr`
# rather than renaming a dozen bindings in a widget held to golden files. The
# alias is invisible to `xgettext`, which matches on the spelling at the call
# site: leaving it out silently dropped twenty messages — the whole tree — from
# a catalogue that still looked healthy, and the count is what caught it.
#
...
xgettext \
    ...
    --sort-by-file \
    --package-name="$domain" \
    --msgid-bugs-address=https://github.com/excelano/flyleaf/issues \
    --output="$pot" \
    $sources
