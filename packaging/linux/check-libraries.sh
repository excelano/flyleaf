...
# declares. Adapted from slipcase-desktop's, where the defect it exists for
# was found; the Windows half of the pair arrives with that platform.
#
...
#
# It is a command and never a test, for the reason slipcase-desktop's corpus
# runner is: it needs a display, and a test that has to choose between skipping quietly
...
# The application needs a file to open, and it must be one that parses: a
# refusal leaves the window up but takes a different path through the code
...
observe() {
    ...
    label=$1
    shift
    "$@" "$binary" "${stage}/sample.toml" >"${stage}/${label}.log" 2>&1 &
    pid=$!
