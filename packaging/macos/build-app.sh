#!/bin/sh
# Assemble the application bundle: the executable, the property list that
# imports the TOML type and claims it, and the icon. Adapted from
# slipcase-desktop's build-app.sh, where every check in it was measured;
# the reasoning behind each is recorded there and not restated here.
#
...
# unsigned bundle carrying `Flyleaf.entitlements` is not sandboxed and proves
...
usage: build-app.sh [--binary PATH] [--outdir DIR] [--universal] [--sign ID]
  ...
  --outdir DIR   where to write "Tommy Flyleaf.app" (default: ./dist)
  ...
  --store PROFILE
                 ...
                 from the developer portal:
                   ...
                   ./packaging/macos/build-app.sh \
                       --store ~/Downloads/Tommy_Flyleaf_Mac_App_Store.provisionprofile
USAGE
}
...
if [ -n "$store_profile" ]; then
    ...
    # that it is one.
    store_plist=$(mktemp -t flyleaf-profile)
...
if [ "$universal" = yes ]; then
    ...
    for triple in x86_64-apple-darwin aarch64-apple-darwin; do
        slice="${target_dir}/${triple}/release/flyleaf"
        [ -x "$slice" ] || {
    ...
    binary="${target_dir}/release/flyleaf-universal"
...
if [ -z "$binary" ]; then
    binary="${target_dir}/release/flyleaf"
fi
[ -x "$binary" ] || {
...
# slipcase-desktop 0.1.1 was refused on 2026-08-31 for referencing
...
stage=$(mktemp -d)
iconset="${stage}/flyleaf.iconset"
mkdir -p "$iconset"
svg="${root}/packaging/linux/icons/flyleaf.svg"
...
iconutil --convert icns "$iconset" --output "${app}/Contents/Resources/flyleaf.icns"
...
install -m 0755 "$binary" "${app}/Contents/MacOS/flyleaf"
...
if [ "$universal" = yes ]; then
    ...
    for arch in x86_64 arm64; do
        ...
        got=$(otool -arch "$arch" -l "${app}/Contents/MacOS/flyleaf" |
...
if [ -n "$store_profile" ]; then
    ...
    store_ents=$(mktemp -t flyleaf-entitlements)
...
if [ -n "$identity" ]; then
    codesign --force --timestamp=none \
        --sign "$identity" \
        --entitlements "${here}/Flyleaf.entitlements" \
        "$app"
...
echo "  open -a \"${app}\" packaging/sample.toml"
