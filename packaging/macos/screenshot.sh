...
# Adapted from slipcase-desktop's; the three measurements below were made there.
#
...
# file to open or whether the result is a good advertisement. What it can
...
#
#   ./packaging/macos/screenshot.sh --app "dist-devid/Tommy Flyleaf.app" \
#       --file packaging/sample.toml --out shots/01-window.png
#
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --file) file="${2:?--file needs a file}"; shift 2 ;;
...
[ -n "$file" ] || refuse "no --file given"
[ -n "$out" ] || refuse "no --out given"
[ -d "$app" ] || refuse "no bundle at $app"
[ -f "$file" ] || refuse "no file at $file"
...
# answers "Unable to find application named 'dist-devid/Tommy Flyleaf.app'" — which
...
let wanted = CommandLine.arguments.count > 1 ? CommandLine.arguments[1] : "Tommy Flyleaf"
guard
...
# holding the file this run was given rather than one left over.
...
tell application "System Events"
    set p to first process whose name contains "flyleaf"
...
id=$(swift "$helper" "Tommy Flyleaf") || refuse "could not find the window"
