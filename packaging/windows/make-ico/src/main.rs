...
/// upscale of it. `flyleaf.ico` carries nine sizes for exactly this reason and
...
/// `flyleaf.ico`. One application with two faces, which is what the manifest's
...
/// and wants the whole of it, which is also what `flyleaf.ico` does at every
/// size it holds. **Only a look at real tiles settles the two thirds**, and
/// that look was taken and it settled them; `git log` has it.
///
/// `flyleaf.png` is the file type association logo and is a single 256, not a
...
const ASSETS: &[Asset] = &[
    ...
    Asset { stem: "flyleaf", width: 256, height: 256, fill: 1.0, icon: false },
];
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let source = args.next().map_or_else(
        || here.join("../../linux/icons/flyleaf.svg"),
        PathBuf::from,
    );
    let destination = args
        .next()
        .map_or_else(|| here.join("../flyleaf.ico"), PathBuf::from);
    let assets = args
        .next()
