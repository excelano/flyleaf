...
.SYNOPSIS
    ...
    Copied from slipcase-desktop, where the measurement below was made; the
    in-box list is that repository's and grows here only when a DLL is
    confirmed present on a stock Windows.

.DESCRIPTION
    slipcase-desktop 0.1.1 failed Microsoft Store certification on 2026-08-29 under
...
if (-not $Binary) {
    ...
    $Binary = Join-Path $here '..\..\target\release\flyleaf.exe'
    ...
    if ($meta) { $Binary = Join-Path $meta.target_directory 'release\flyleaf.exe' }
}
...
if ($unknown.Count -gt 0) {
    ...
    Write-Host 'Tommy Flyleaf will start, and a Store tester will have a clean machine.' -ForegroundColor Yellow
