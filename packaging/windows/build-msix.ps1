...
# DESIGN.md §9 carries what came back.
#
...
function Test-CertificationReport([string] $report) {
    ...
    foreach ($test in $xml.SelectNodes('//TEST')) {
        ...
        if ($expected -eq $verdict) {
            Write-Host "$verdict  $name  (known - see DESIGN.md)"
        } else {
...
if (-not $Binary) {
    ...
    $Binary = Join-Path $targetDir 'release\flyleaf.exe'
}
...
# is a certification tester's rather than this one. slipcase-desktop 0.1.1 was packaged,
...
Copy-Item $Binary (Join-Path $stage 'flyleaf.exe')
...
foreach ($image in 'StoreLogo.png', 'Square150x150Logo.png', 'Square44x44Logo.png',
                   'Wide310x150Logo.png', 'flyleaf.png') {
...
$package = Join-Path $OutDir "TommyFlyleaf-$version-x64.msix"
...
if ($SelfSign) {
    ...
    if (-not $cert) {
        ...
        $cert = New-SelfSignedCertificate -Type CodeSigningCert `
            ...
            -FriendlyName 'Tommy Flyleaf MSIX test signing (throwaway)' `
    ...
    if ($trusted) {
        ...
        Write-Host "  Export-Certificate -Cert Cert:\CurrentUser\My\$($cert.Thumbprint) -FilePath `$env:TEMP\flyleaf-test.cer"
        Write-Host '  # then, from an elevated prompt:'
        Write-Host "  Import-Certificate -FilePath `$env:TEMP\flyleaf-test.cer -CertStoreLocation Cert:\LocalMachine\TrustedPeople"
        Write-Host "  Add-AppxPackage $package"
    }
}
