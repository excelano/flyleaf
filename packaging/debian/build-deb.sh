...
# the desktop entry, the application icon, the manual page and the changelog.
# Adapted from slipcase-desktop's, which is where every decision in it was
# measured; the media type is the desktop's own here, so there is no sibling
# package to depend on for it.
#
...
mkdir -p \
    ...
    "${stage}/usr/share/doc/flyleaf"

install -m 0755 "$binary" "${stage}/usr/bin/flyleaf"
install -m 0644 "${here}/../linux/flyleaf.desktop" \
    "${stage}/usr/share/applications/flyleaf.desktop"
install -m 0644 "${here}/../linux/icons/flyleaf.svg" \
    "${stage}/usr/share/icons/hicolor/scalable/apps/flyleaf.svg"
install -m 0644 "${root}/LICENSE" "${stage}/usr/share/doc/flyleaf/copyright"
...
# repository has no other way to see what changed between two versions.
#
...
gzip -9nc "${here}/changelog" \
    > "${stage}/usr/share/doc/flyleaf/changelog.gz"
chmod 0644 "${stage}/usr/share/doc/flyleaf/changelog.gz"

# `flyleaf` reads one optional positional argument and has no
...
sed "s/@VERSION@/${version}/" "${here}/flyleaf.1.in" \
    | gzip -9nc > "${stage}/usr/share/man/man1/flyleaf.1.gz"
chmod 0644 "${stage}/usr/share/man/man1/flyleaf.1.gz"
...
strip --strip-unneeded "${stage}/usr/bin/flyleaf" 2>/dev/null || true
